...
public class OperationalProductionPlanService {
	...
	public List<OperationalProductionPlanDTO> getAllData() {
		...
		List<ProductOperationalProductionPlan> productplan = pps.getAllPP();
		
		
		for (ProductOperationalProductionPlan prplan : productplan) {
			for (OperationalProductionPlanRepository plan : rep) {
				if(plan.getId()==rep.getOne(id)) {
					OperationalProductionPlanDTO newProductPlan = new OperationalProductionPlanDTO
							(productplan.getProductKey(), product.getProductKey(),product.getNameProduct(),product.getPriceProduct(),warehouseProduct.getQuantity());
